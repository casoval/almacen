...
class ProveedorAdmin(admin.ModelAdmin):
    list_display = ('nombre', 'direccion', 'telefono', 'email', 'activo', 'get_uso_estado')
    ...
    def get_uso_estado(self, obj):
        """Muestra si el proveedor está siendo usado en movimientos"""
        if obj.pk:
            from almacenes.models import MovimientoAlmacen
            from beneficiarios.models import MovimientoCliente
            
            en_uso = (
                MovimientoAlmacen.objects.filter(proveedor=obj).exists() or
                MovimientoCliente.objects.filter(proveedor=obj).exists()
            )
    ...
    def save_model(self, request, obj, form, change):
        """Mensaje al guardar proveedor"""
        try:
            super().save_model(request, obj, form, change)
            accion = 'actualizado' if change else 'creado'
            messages.success(request, f'✅ Proveedor "{obj.nombre}" {accion} correctamente.')
        except Exception as e:
            messages.error(request, f'❌ Error al guardar el proveedor: {str(e)}')
            raise
    
    def delete_model(self, request, obj):
        """Mensaje al eliminar un proveedor"""
        nombre = obj.nombre
        try:
